							 ...
							 pjsip_rx_data *rdata)
{
	...
	sprintf(call_info->incoming, "Incoming call from %.*s!!",
...
void demo_pj_answer(void) {
	...
	if (current_call != PJSUA_INVALID_ID) {
		...
		sprintf(call_info->remote_uri, "Remote URI: %s", ci.remote_info.ptr);
		sprintf(call_info->remote_contact, "Remote contact: %s", ci.remote_contact.ptr);
 ...
 */
int demo_pj_main(int argc, char *argv[], int (*nk_cb)(void))
{
	pjsua_acc_id acc_id;
	pj_status_t status;
